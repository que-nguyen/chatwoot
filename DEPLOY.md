...
- `CW_IMAGE_TAG`: tuỳ chọn; mặc định `latest`. Nếu pin version mà gặp lỗi `manifest unknown`, đổi về `latest` hoặc bỏ biến này (preflight sẽ FAIL sớm khi tag không tồn tại trên registry).
